...
static void handle_find_value(struct pollfd* sock, struct RPCFind* data) {
    ...
    log_msg(LOG_DEBUG, "We don't have the key value pair, returning our neighbors closest to target");
...
void handle_rpc_download(struct FileMagnet* file) {
    ...
    // Vector to store for each of the closest nodes whether we already contacted them or not
    VectorPtr contacted;
    ...
    while (!value_found) {
        // Whether at least one new peer was contacted during this loop, if not we can assume we got as close as possible to our target but no one has it
            ...
            *c = true;
            ...
            if (get_rpc_request(&pollfd_sock, buf, &packet_size) == 0) {
                ...
                if (header->call_type == FIND_VALUE_RESPONSE) {
                    struct RPCFindValueResponse* response = (struct RPCFindValueResponse*)buf;
                    ...
                    if (response->found_key) {
                        ...
                        // They didn't have the key, get their closest peers and add to our list
                        for (int j = 0; j < response->num_closest; j++) {
                            ...
                            for (size_t s = 0; s < closest.size; s++) {
                                ...
                                if (q && memcmp(&q->peer_id, &response->closest[j].peer_id, sizeof(HashID)) == 0) {
