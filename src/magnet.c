...
struct FileMagnet *create_magnet(const char *filename, size_t filename_len) {
  ...
  // Extract basename (filename only) from the path
  const char *base_name = strrchr(filename, '/');
  base_name = (base_name) ? base_name + 1 : filename;
  size_t base_len = strlen(base_name);

  new_magnet->display_name = (char *)malloc(base_len + 1);
  pointer_not_null(new_magnet->display_name,
                   "Error in create_magnet display_name alloc failed!\n");
  memcpy(new_magnet->display_name, base_name, base_len);
  new_magnet->display_name[base_len] = '\0';
