...
ssize_t recv_until(int fd, void* dst, size_t buf_len, const char* pattern,
                   size_t pattern_len) {
  size_t nb_read = 0;
  ssize_t ret = 0;
...
int get_primary_ip(char* ip_buf, size_t buf_size,
  ...
  if (cached) {
    ...
    if (out_addr) memcpy(out_addr, &cached_addr, sizeof(struct sockaddr_in));
    return 0;
  }
  ...
  if (out_addr) memcpy(out_addr, &name, sizeof(struct sockaddr_in));
